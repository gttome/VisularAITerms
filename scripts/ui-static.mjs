...
const problems=[];if(!html.includes(`v${version}`))problems.push(`src/index.html missing v${version}`);const start=await fs.readFile(path.join(root,'start-server.bat'),'utf8');if(!start.includes(`v${version}`))problems.push(`start-server.bat missing v${version}`);
...
console.log(`UI static integrity: PASS — ${ids.length} unique IDs; all querySelector ID targets exist; version markers match v${version}.`);
