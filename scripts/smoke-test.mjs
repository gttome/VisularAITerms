...
try{
  await check('/');const catalog=await (await check('/data/catalog.json')).json();const learningPaths=await (await check('/data/learning-paths.json')).json();if(!Array.isArray(learningPaths.learningPaths))throw new Error('learning paths payload missing');if(catalog.application?.version!=='0.5.0')throw new Error('catalog version mismatch');if((catalog.concepts||[]).length<2)throw new Error('expected supplied concepts');
